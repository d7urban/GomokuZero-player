...
BEST_WEIGHTS = "gomoku_best.weights.h5"
CHECKPOINT_GLOB = "gomoku_*.weights.h5"
...
def select_weights(mode="best", explicit_path=""):
    ...
    Mode may be:
      - "best" (default): prefer the included best weights, then newest checkpoint
      - "latest": alias for "best" for compatibility
    ...
    if os.path.exists(BEST_WEIGHTS):
        return BEST_WEIGHTS, "included"

    # Last resort: newest checkpoint file in the repository root.
    files = [
        path for path in glob.glob(CHECKPOINT_GLOB)
        if os.path.basename(path) != BEST_WEIGHTS
    ]
    if files:
